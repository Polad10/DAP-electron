...
{
    let treatments = pagination.paginate($(this), 'treatment_pagination', currentTreatments);
    ...
    else
    {
        currentTreatments = allTreatments.filter(t => {
            if(names.length === 1)
            {
                ...
                return t.first_name.toLowerCase().includes(name) || t.last_name.toLowerCase().includes(name);
            }
            else if(names.length)
            {
                ...
                return (t.first_name.toLowerCase().includes(name_1) && t.last_name.toLowerCase().includes(name_2))
                        || (t.first_name.toLowerCase().includes(name_2) && t.last_name.toLowerCase().includes(name_1));
            }
        });
    }
